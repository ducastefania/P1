...
Original file is located at
    https://colab.research.google.com/drive/1NwMwSyi3fKGzq2x4un_g1eyjrFU30ZME
"""
...
def generate_random_var(mean, stDev, noSamples, noVariables):
  ...
  for i in range(1, noVariables):
    ...
    while True:
      ...
      coef1 = random.uniform(1, 2)
      coef2 = random.uniform(1, 4)
      #
...
def generate_random_var_all(mean, stDev, noSamples, noVariables):
  ...
  for i in range(1, noVariables):
    ...
    coef_mean = random.uniform(0.75, 1.1)
    coef_stDev = random.uniform(0.5, 1.1)
    aux = np.random.normal(loc=coef_mean, scale=coef_stDev, size=noSamples) + np.array(result[i-1])
    result.append(aux.tolist())
    coefs.append([coef1, coef2])
...
def generate_random_var_with_hidden_data(mean, stDev, noSamples, noVariables):

  result = list()
  coefs = list()
  replacement_list = [1, 4, 8]

  random_mean = random.uniform(0.75, 1.1)
  random_stDev = random.uniform(0.5, 1.1)
  z = np.random.normal(loc=random_mean, scale=random_stDev, size=noSamples)
  ...
  for i in range(1, noVariables):
    ...
    aux = np.random.normal(loc=mean, scale=stDev, size=noSamples) + np.array(result[i-1])

    if(i in replacement_list):
      aux += z
...
def generate_random_var_missig_data(mean, stDev, noSamples, noVariables, percentage):
  ...
  while no_replacements > 0:
    ...
    random_i =  random.randint(1, noVariables - 1)
    random_j = random.randint(1, noSamples - 1)
    result[random_i, random_j] = 0
...
def generate_random_var_noise_data(mean, stDev, noSamples, noVariables, percentage):
  ...
  while no_replacements > 0:
    ...
    random_mean = random.uniform(0.1, 1)
    random_stDev = random.uniform(0.1, 1)

    random_i =  random.randint(1, noVariables - 1)
    random_j = random.randint(1, noSamples - 1)
    random_var =  np.random.normal(loc=random_mean, scale=random_stDev, size=noSamples)
    random_position = random.randint(1, noSamples - 1)

    result[random_i, random_j] = result[random_i, random_j] + random_var[random_position]
    no_replacements = no_replacements - 1
...
def acc_func(result, test_chain):
  ...
  if(result == test_chain or result == test_chain.reverse()):
...
def case1_test(mean, stDev, noSamples, noVariables):
  ...
  return mi_acc, corr_acc


# case 2: random stDev, random mean - not working
...
# case 6: add z noise at data generation
def case6(mean, stDev, noSamples, noVariables):

  data = list()
  data, coefs= generate_random_var_with_hidden_data(mean, stDev, noSamples, noVariables)
  print(f"Data:\n {data}")
  print(f"Coefs:\n {coefs}")
...
# case 1: generate dataset with constant mean, stDev

# compare the following:
  # how accuracy changes when we increase no of samples and no of variables
  # test for a couple of different combinations of mean, stDev
...
stDev_list = list()
noSamples_list = list()
noVariables_list = list()

# 1. mean = 0, stDev = 1; take 5 examples with noSamples in (100, 1000) and noVariables in (5, 100)

# declare (starting) constants
mean = 0
stDev = 1
noSamples = 100
noVariables = 5
...
while successful_runs < 5:
    ...
    try:
        mi_acc, corr_acc = case1_test(mean, stDev, noSamples, noVariables)

        # Save data
        mean_list.append(mean)
        stDev_list.append(stDev)
        noSamples_list.append(noSamples)
        noVariables_list.append(noVariables)
        mi_acc_list.append(mi_acc)
        corr_acc_list.append(corr_acc)
        print("Case 1 successfully computed")
        successful_runs += 1

        # update variables
        noSamples = noSamples + 50
        noVariables = noVariables * 2

    except Exception as e:
        print("Bad data set - generating new one:", e)

    i += 1

x_labels = [f"({noSamples}, {noVariables})" for noSamples, noVariables in zip(noSamples_list, noVariables_list)]
plt.figure(figsize=(10, 6))
plt.bar(x_labels, mi_acc_list, color='blue')

plt.xlabel('no of Sample and no ofVariables (mutual information)')
plt.ylabel('Accuracy (mutual information)')
plt.title('Bar Plot of Accuracy vs. Sample - Variable Pairs')

x_labels = [f"({noSamples}, {noVariables})" for noSamples, noVariables in zip(noSamples_list, noVariables_list)]
plt.figure(figsize=(10, 6))
plt.bar(x_labels, corr_acc_list, color='blue')

plt.xlabel('no of Sample and no ofVariables (correlation)')
plt.ylabel('Accuracy (correlation)')
plt.title('Bar Plot of Accuracy vs. Sample  Var-iable Pairs')
...
# while True:
#   try:
#     case2(mean, stDev, noSamples, noVariables, percent)
#     print("Case 2 successfully computed")
#     break
#   except Exception as e:
#             print("Generating new data set")
...
#     print("Case 3 successfully computed")
#     break
#   except Exception as e:
#             print("Generating new data set")
...
#     print("Case 4 successfully computed")
#     break
#   except Exception as e:
#             print("Generating new data set")
...
#     print("Case 5 successfully computed")
#     break
#   except Exception as e:
#             print("Generating new data set")
...
# while True:
#   try:
#     case6(mean, stDev, noSamples, noVariables)
#     print("Case 6 successfully computed")
#     break
#   except Exception as e:
#             print("Generating new data set")
